...
(inpt_training, outpt_training), (inpt_validation, outpt_validation) = mnist.load_data()



model = k.models.Sequential([
    k.layers.Conv2D(5, (2,2), input_shape=(None, imageWidth, imageHeight), name="conv1"),
    k.layers.MaxPool2D(),
    k.layers.Conv2D(5, (2,2), name="conv2"),
    k.layers.MaxPool2D(),
    k.layers.Conv2D(5, (2,2), name="conv3"),
    k.layers.MaxPool2D(),
    k.layers.Flatten(),
...
modelSaver = k.callbacks.ModelCheckpoint(
    tfDataDir + "mnistPredModel_checkpoint.h5", 
    monitor="val_loss", 
    mode="min",
    save_best_only=True
)
...
model.save("{}/mnistModel.h5".format(resultDir))
model.save("{}/mnistModel.h5".format(tfDataDir))
